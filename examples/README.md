...
Worked examples wiring the scanner to PDPs. The **mock PDP, OpenFGA, and Cedar examples
are runnable** (each with a `smoke.sh` and a Docker-gated integration test); AVP and the
scenario walk-through are still placeholders (see [`ROADMAP.md`](../ROADMAP.md) M3).
